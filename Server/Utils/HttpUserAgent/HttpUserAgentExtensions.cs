...
public static partial class HttpUserAgentExtensions
{
    ...
    {
        public UserAgent? DetectUserAgent()
        {
            ...
            var matchMSWebDAV = MicrosoftWebDav.Match(raw);
            if (matchMSWebDAV.Success)
            {
                return new UserAgent { Raw = raw, CalendarClientType = CalendarClientType.MicrosoftWebDAV, Platform = Platform.NotDetected, };
            }

            var matchMacWebDav = MacWebDav.Match(raw);
            if (matchMacWebDav.Success)
            {
                return new UserAgent { Raw = raw, CalendarClientType = CalendarClientType.MacOSWebDAV, Platform = Platform.NotDetected, };
            }
    ...
    private static partial Regex MacCalendarRegex { get; }
    ...
    private static partial Regex EMClientRegex { get; }
    ...
    private static partial Regex DAVx5Regex { get; }
    ...
    private static partial Regex ThunderbirdRegex { get; }

    // OneCalendar Android APP
    //      User-Agent: curl/7.54

    // Mac/WebDAV
    //  User-Agent: WebDAVFS/3.0.0 (03008000) Darwin/25.5.0 (arm64)
    //  User-Agent: WebDAVLib/1.3
    [GeneratedRegex(@"WebDAV(FS|Lib)\/(?'version'[\d.]+)", RegexOptions.IgnoreCase | RegexOptions.ExplicitCapture, matchTimeoutMilliseconds: 200)]
    private static partial Regex MacWebDav { get; }

    // Microsoft-WebDAV-MiniRedir/10.0.26200
    [GeneratedRegex(@"Microsoft-WebDAV-MiniRedir\/(?'version'[\d.]+)", RegexOptions.IgnoreCase | RegexOptions.ExplicitCapture, matchTimeoutMilliseconds: 200)]
    private static partial Regex MicrosoftWebDav { get; }
}
