...
/// Support for COPY for CALDAV resources is removed due to missing calendar client support
/// </summary>
/// <remarks>
/// The specification of the COPY method can be found in the <see cref="https://datatracker.ietf.org/doc/html/rfc4918#section-9.8"/>WebDav specification</see>.
/// </remarks>
public class CopyHandler(DavEnvironmentRepository env, MoveCopyRepository MoveRepository, ResourceRepository ResourceRepository, RecorderSession recorder) : HandlerBase(env, recorder), IMethodHandler
{
    public async Task HandleRequestAsync(HttpContext httpContext, DavResource resourceSource)
    {
        ...
        switch (resourceSource.ResourceType)
        {
            ...
            case DavResourceType.User:
                Log.Error("COPY on this resource type {uri} not supported", request.GetEncodedUrl());
                await WriteStatusAsync(httpContext, HttpStatusCode.Forbidden);
                return;
            default:
                break;
        ...
        switch (resourceSource.ResourceType)
        {
            ...
            case DavResourceType.AddressbookItem:
                // This is currently an intended limitation of this implementation mostly due to lack of calendar client support
            ...
            case DavResourceType.BlobItem:
                {
                    ...
                    var copyObject = await MoveRepository.PrepareCopyToAsync(resourceSource.Object, resourceTarget, resourceTarget.Object, storage, httpContext.RequestAborted);
                    ...
                    await MoveRepository.CommitCopyToAsync(resourceSource.Object, resourceTarget.Object, httpContext.RequestAborted);
                    ...
                    await WriteStatusAsync(httpContext, !resourceTarget.Exists ? HttpStatusCode.Created : HttpStatusCode.NoContent);
                }
                return;
            ...
            case DavResourceType.Container:
                {
                    ...
                    if (resourceTarget.ParentResourceType != DavResourceType.Principal && resourceTarget.Parent is null)
                    {
                        // missing intermediate collections => Conflict
                        await WriteErrorXmlAsync(httpContext, HttpStatusCode.Conflict, Precondition.CollectionMustExist, "The destination collection must exist.");
                        return;
                    ...
                    var copyCollections = await MoveRepository.PrepareCopyToAsync(resourceSource.Current, resourceTarget.Parent?.Id ?? resourceTarget.Owner.Id, resourceTarget.Uri, resourceTarget.Current, storage, httpContext.RequestAborted);
                    if (!await storage.Prepare(httpContext.RequestAborted))
                    {
                        await WriteStatusAsync(httpContext, HttpStatusCode.InternalServerError, "Copy in storage failed");
                        return;
                    }
                    await MoveRepository.CommitCopyToAsync(copyCollections, resourceTarget.Current, httpContext.RequestAborted);
                    if (!await storage.Commit(httpContext.RequestAborted))
                    {
                        Log.Fatal("Storage copy failed during commit-phase. Storage potentially is now inconsistent.");
                        await WriteStatusAsync(httpContext, HttpStatusCode.InternalServerError, "Copy in storage severely failed, inconsistent state possible");
                    ...
                    SetLocation(response, resourceTarget.DavName);
                    await WriteStatusAsync(httpContext, resourceTarget.Current is null ? HttpStatusCode.Created : HttpStatusCode.NoContent);
                    // await WriteStatusAsync(httpContext, HttpStatusCode.NotImplemented, "COPY on this resource type is not implemented");
                }
                return;
